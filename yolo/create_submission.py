...
def main():
    ...
    for img_id, img in imgs:
        ...
        boxes = get_prediction(models, img, use_fragmentation=True, fragment_size = 640*2, intersection_th = 0.3)
    ...
    json.dump(
            coco_results,
            (pathlib.Path(__file__).parent / f"submission_{v}.json").open("w"),
            indent=4,
        )
